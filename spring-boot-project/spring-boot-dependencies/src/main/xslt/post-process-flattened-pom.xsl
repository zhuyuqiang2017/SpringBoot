...
<xsl:stylesheet version="2.0"
	...
	<xsl:template
		match="/m:project/m:dependencyManagement/m:dependencies/m:dependency/m:version/text()[. = '2.1.7.BUILD-SNAPSHOT']">
		<xsl:value-of select="/m:project/m:version/text()" />
	</xsl:template>
	<xsl:template
		match="/m:project/m:build/m:pluginManagement/m:plugins/m:plugin/m:version/text()[. = '2.1.7.BUILD-SNAPSHOT']">
		<xsl:value-of select="/m:project/m:version/text()" />
	</xsl:template>
